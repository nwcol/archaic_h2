...
def get_args():
    ...
    parser.add_argument("-n", "--chromosome_number")
...
def count_sites():
    ...
    for i, window in enumerate(windows):
        site_counts[i] = len(one_locus.slice_window(positions, window))
...
if __name__ == "__main__":
    ...
    positions = one_locus.mask_positions_from_regions(mask_regions)
    vcf_pos, sample_names, genotypes = one_locus.read_vcf_file(
        args.vcf_fname, mask_regions=mask_regions
    )
    pos_map = two_locus.get_map_vals(args.map_fname, positions)
