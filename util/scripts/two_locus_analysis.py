...
def get_header_dict(chrom, stat, window, window_id, n_sites, rows):
    header_dict = {
        "chr": chrom,
        "stat": stat,
        "window": window,
...
def main(window, window_id):
    ...
    n_sites = sample_set.interval_n_positions(window[0], window[1])
    ...
    header_dict = get_header_dict(chrom, "H2", window, window_id, n_sites, rows)
    ...
    header_dict = get_header_dict(chrom, "H2_2", window, window_id, n_sites, rows)
