...
def main():
    ...
    for map_fname, mask_fname in zip(args.u_maps, args.masks):
        ...
        tot_u += u_sum
        print(
            f'chromosome {chrom_num}\n'
            f'num sites {num_sites}\n'
            f'avg u {np.format_float_scientific(u_sum / num_sites, 4)}\n\n'
        )
    print(
        util.get_time(), f'read mutation rate in {len(args.masks)} files\n'
        f'num sites {tot_num_sites}\n'
        f'avg u {np.format_float_scientific(tot_u / tot_num_sites, 4)}\n'
    )
    return 0
