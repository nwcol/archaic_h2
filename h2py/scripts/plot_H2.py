...
def plot_H2stats(
    ...
    axs[n_axs - 2].set_xticks(
        np.arange(len(hlabels)), labels=hlabels, rotation=90
    )
    axs[n_axs - 1].set_xticks(
        np.arange(len(hxylabels)), labels=hxylabels, rotation=90
    )
    return l
...
def main():
    ...
    # load statistics
    graph0 = args.graph_files[0] if len(args.graph_files) > 0 else None
    datas = []
    data_labels = []
    for file in args.data_files:
        instances = h2stats_mod.load_data(file, graph=graph0)
        for key in instances:
            label = os.path.basename(file) + '-' + key
            data_labels.append(label)
            datas.append(instances[key])

    # load graphs
    if len(datas) > 0:
        data0 = datas[0]
        num_stats = data0.stats.shape[1]
        bins = data0.bins
        models = [H2stats.from_demes(g, u, template=data0) 
                  for g in args.graph_files]
        lls = [inference.compute_ll(m, data0, include_H=False)
               for m in models]
        
    else:
        bins = h2stats_mod._default_bins
        models = [H2stats.from_demes(g, u) for g in args.graph_files]
        num_stats = models[0].stats.shape[1]
        lls = None
    ...
    if len(datas + models) <= 10:
        colors = Category10[10]
    else:
        n = len(datas + models)
        gap = int(np.floor(256 / n))
    ...
    if lls is not None:
        graph_labels = [os.path.basename(args.graph_files[i]) + 
                   f', ll={np.round(lls[i], 2)}'
                   for i in range(len(args.graph_files))]
    else:
        graph_labels = [os.path.basename(g) for g in args.graph_files]
    labels = data_labels + graph_labels
    y_anchor = -0.2 / n_rows
    plt.figlegend(
        lines, 
        labels, 
